...
![图像识别四大类任务，图像来源于cs231n 2016课件Lecture 8](https://cdn.jsdelivr.net/gh/makaspacex/PictureZone@main/libs/dlwarm/img/ch8/8.1.1.png)
...
![](https://cdn.jsdelivr.net/gh/makaspacex/PictureZone@main/libs/dlwarm/img/ch8/8.1.2.png)
...
![](https://cdn.jsdelivr.net/gh/makaspacex/PictureZone@main/libs/dlwarm/img/ch8/8.2.1-1.png)
...
![](https://cdn.jsdelivr.net/gh/makaspacex/PictureZone@main/libs/dlwarm/img/ch8/8.1.4.png)

4. 训练SVM。利用feature map 训练SVM来对目标和背景进行分类（每个类一个二进制SVM）

5. 边界框回归（Bounding boxes Regression）。训练将输出一些校正因子的线性回归分类器

![](https://cdn.jsdelivr.net/gh/makaspacex/PictureZone@main/libs/dlwarm/img/ch8/8.1.5.png)

**R-CNN 实验结果**

R-CNN在VOC 2007测试集上mAP达到58.5%，打败当时所有的目标检测算法。

![](https://cdn.jsdelivr.net/gh/makaspacex/PictureZone@main/libs/dlwarm/img/ch8/8.1.6.png)
...
![](https://cdn.jsdelivr.net/gh/makaspacex/PictureZone@main/libs/dlwarm/img/ch8/8.2.2-1.png)
...
![](https://cdn.jsdelivr.net/gh/makaspacex/PictureZone@main/libs/dlwarm/img/ch8/8.2.2-2.png)
...
![](https://cdn.jsdelivr.net/gh/makaspacex/PictureZone@main/libs/dlwarm/img/ch8/8.1.11.gif)
...
![](https://cdn.jsdelivr.net/gh/makaspacex/PictureZone@main/libs/dlwarm/img/ch8/8.2.3-1.png) 

图8.1.13 Faster R-CNN的流程图
Faster R-CNN的流程图与Fast R-CNN相同，采用外部候选区域方法代替了内部深层网络。
![](https://cdn.jsdelivr.net/gh/makaspacex/PictureZone@main/libs/dlwarm/img/ch8/8.2.3-2.png) 
...
![](https://cdn.jsdelivr.net/gh/makaspacex/PictureZone@main/libs/dlwarm/img/ch8/8.2.3-3.png) 
图8.1.15

对于特征图中的每一个位置，RPN会做k次预测。因此，RPN将输出4×k个坐标和每个位置上2×k个得分。下图展示了8×8的特征图，且有一个3×3的卷积核执行运算，它最后输出8×8×3个ROI（其中k=3）。下图（右）展示了单个位置的3个候选区域。
![](https://cdn.jsdelivr.net/gh/makaspacex/PictureZone@main/libs/dlwarm/img/ch8/8.2.3-4.png)
图8.1.16

假设最好涵盖不同的形状和大小。因此，Faster R-CNN不会创建随机边界框。相反，它会预测一些与左上角名为锚点的参考框相关的偏移量（如x, y）。我们限制这些偏移量的值，因此我们的猜想仍然类似于锚点。
![](https://cdn.jsdelivr.net/gh/makaspacex/PictureZone@main/libs/dlwarm/img/ch8/8.1.17.png) 
图8.1.17

要对每个位置进行k个预测，我们需要以每个位置为中心的k个锚点。每个预测与特定锚点相关联，但不同位置共享相同形状的锚点。
![](https://cdn.jsdelivr.net/gh/makaspacex/PictureZone@main/libs/dlwarm/img/ch8/8.2.3-6.png) 
图8.1.18

这些锚点是精心挑选的，因此它们是多样的，且覆盖具有不同比例和宽高比的现实目标。这使得我们可以用更好的猜想来指导初始训练，并允许每个预测专门用于特定的形状。该策略使早期训练更加稳定和简便。
![](https://cdn.jsdelivr.net/gh/makaspacex/PictureZone@main/libs/dlwarm/img/ch8/8.2.3-7.png)
图8.1.19
...
![](https://cdn.jsdelivr.net/gh/makaspacex/PictureZone@main/libs/dlwarm/img/ch8/8.2.4-1.png)
图8.2.1 人脸检测
...
![](https://cdn.jsdelivr.net/gh/makaspacex/PictureZone@main/libs/dlwarm/img/ch8/8.2.4-2.png) 
图8.2.2 检测示例
我们将方块分成9个部分，由此创建了9个特征图，每个用来检测对应的目标区域。这些特征图叫做位置敏感得分图（position-sensitive score map），因为每个图检测目标的子区域（计算其得分）。
![](https://cdn.jsdelivr.net/gh/makaspacex/PictureZone@main/libs/dlwarm/img/ch8/8.2.4-3.png)
图8.2.3生成9个得分图
...
![](https://cdn.jsdelivr.net/gh/makaspacex/PictureZone@main/libs/dlwarm/img/ch8/8.2.4-4.png) 
图8.2.4

将ROI应用到特征图上，输出一个3x3数组。将得分图和ROI映射到vote数组的过程叫做位置敏感ROI池化（position-sensitive ROI-pool）。该过程与前面讨论过的ROI池化非常接近。
![](https://cdn.jsdelivr.net/gh/makaspacex/PictureZone@main/libs/dlwarm/img/ch8/8.2.4-5.png) 
图8.2.5

将ROI的一部分叠加到对应的得分图上，计算V[i][j]。在计算出位置敏感ROI池化的所有值后，类别得分是其所有元素得分的平均值。
![](https://cdn.jsdelivr.net/gh/makaspacex/PictureZone@main/libs/dlwarm/img/ch8/8.2.6.png)
图8.2.6 ROI池化
...
![](https://cdn.jsdelivr.net/gh/makaspacex/PictureZone@main/libs/dlwarm/img/ch8/8.2.7.png) 
图8.2.7
...
![](https://cdn.jsdelivr.net/gh/makaspacex/PictureZone@main/libs/dlwarm/img/ch8/FPN-01.png)
...
![](https://cdn.jsdelivr.net/gh/makaspacex/PictureZone@main/libs/dlwarm/img/ch8/FPN-02.png)
...
![](https://cdn.jsdelivr.net/gh/makaspacex/PictureZone@main/libs/dlwarm/img/ch8/FPN-03.png)
...
![](https://cdn.jsdelivr.net/gh/makaspacex/PictureZone@main/libs/dlwarm/img/ch8/FPN-04.png)
...
![](https://cdn.jsdelivr.net/gh/makaspacex/PictureZone@main/libs/dlwarm/img/ch8/Mask R-CNN-01.png)
...
![](https://cdn.jsdelivr.net/gh/makaspacex/PictureZone@main/libs/dlwarm/img/ch8/Mask R-CNN-02.png)

为了解决RoI Pooling的上述缺点，作者提出了RoI Align这一改进的方法(如图2)。

![](https://cdn.jsdelivr.net/gh/makaspacex/PictureZone@main/libs/dlwarm/img/ch8/Mask R-CNN-03.png)
...
![](https://cdn.jsdelivr.net/gh/makaspacex/PictureZone@main/libs/dlwarm/img/ch8/Mask R-CNN-04.png)
...
![](https://cdn.jsdelivr.net/gh/makaspacex/PictureZone@main/libs/dlwarm/img/ch8/DetNet-1.png)

实现细节如下图：

![](https://cdn.jsdelivr.net/gh/makaspacex/PictureZone@main/libs/dlwarm/img/ch8/DetNet-2.png)
...
![](https://cdn.jsdelivr.net/gh/makaspacex/PictureZone@main/libs/dlwarm/img/ch8/DetNet-3.png)

* 在FPN基础上明显有大物体涨点，同时由于高分辨率，小物体也有不错的提升。
* 膨胀卷积提供的大感受野使得分类也不逊色
![](https://cdn.jsdelivr.net/gh/makaspacex/PictureZone@main/libs/dlwarm/img/ch8/DetNet-4.png)

3.2  **Results analysis**
![](https://cdn.jsdelivr.net/gh/makaspacex/PictureZone@main/libs/dlwarm/img/ch8/DetNet-5.png)
...
![](https://cdn.jsdelivr.net/gh/makaspacex/PictureZone@main/libs/dlwarm/img/ch8/DetNet-6.png)
...
![](https://cdn.jsdelivr.net/gh/makaspacex/PictureZone@main/libs/dlwarm/img/ch8/DetNet-7.png)

验证方法是做了实验，将P6开始的block换成上图所示的A模块对比效果如下图。 发现还是加了B效果更好。（但是这个stage和传统意义很不一样，所以很多性质不能相提并论，只是B模块的改变也不好判定什么）
![](https://cdn.jsdelivr.net/gh/makaspacex/PictureZone@main/libs/dlwarm/img/ch8/DetNet-8.png)
...
2. **Proposed method**
![](https://cdn.jsdelivr.net/gh/makaspacex/PictureZone@main/libs/dlwarm/img/ch8/CBNet-1.png)
2.1  **Architecture of CBNet**  
![](https://cdn.jsdelivr.net/gh/makaspacex/PictureZone@main/libs/dlwarm/img/ch8/CBNet-2.png)
...
![](https://cdn.jsdelivr.net/gh/makaspacex/PictureZone@main/libs/dlwarm/img/ch8/CBNet-3.png)
...
* **result**  
![](https://cdn.jsdelivr.net/gh/makaspacex/PictureZone@main/libs/dlwarm/img/ch8/CBNet-4.png)
...
![](https://cdn.jsdelivr.net/gh/makaspacex/PictureZone@main/libs/dlwarm/img/ch8/CBNet-5.png)
...
![](https://cdn.jsdelivr.net/gh/makaspacex/PictureZone@main/libs/dlwarm/img/ch8/CBNet-6.png)
...
![](https://cdn.jsdelivr.net/gh/makaspacex/PictureZone@main/libs/dlwarm/img/ch8/CBNet-7.png)
* 
从中可以看出其实权重是否share区别不大， 不到一个点的降幅，参数量减少。


* **Effectiveness of basic feature enhancement by CBNet**   
![](https://cdn.jsdelivr.net/gh/makaspacex/PictureZone@main/libs/dlwarm/img/ch8/CBNet-8.png)
...
![](https://cdn.jsdelivr.net/gh/makaspacex/PictureZone@main/libs/dlwarm/img/ch8/SSD-01.png)
...
![](https://cdn.jsdelivr.net/gh/makaspacex/PictureZone@main/libs/dlwarm/img/ch8/SSD-02.png)
...
![](https://cdn.jsdelivr.net/gh/makaspacex/PictureZone@main/libs/dlwarm/img/ch8/DSSD-01.png)
...
![](https://cdn.jsdelivr.net/gh/makaspacex/PictureZone@main/libs/dlwarm/img/ch8/DSSD-02.png)
...
![](https://cdn.jsdelivr.net/gh/makaspacex/PictureZone@main/libs/dlwarm/img/ch8/DSSD-03.png)
...
![](https://cdn.jsdelivr.net/gh/makaspacex/PictureZone@main/libs/dlwarm/img/ch8/YOLOv1-01.png)
...
![](https://cdn.jsdelivr.net/gh/makaspacex/PictureZone@main/libs/dlwarm/img/ch8/YOLOv1-02.png)
...
![](https://cdn.jsdelivr.net/gh/makaspacex/PictureZone@main/libs/dlwarm/img/ch8/YOLOv1-03.png)
...
![](https://cdn.jsdelivr.net/gh/makaspacex/PictureZone@main/libs/dlwarm/img/ch8/YOLOv1-04.png)
...
![](https://cdn.jsdelivr.net/gh/makaspacex/PictureZone@main/libs/dlwarm/img/ch8/YOLOv2-01.png)
...
![](https://cdn.jsdelivr.net/gh/makaspacex/PictureZone@main/libs/dlwarm/img/ch8/YOLOv2-02.png)
...
![](https://cdn.jsdelivr.net/gh/makaspacex/PictureZone@main/libs/dlwarm/img/ch8/YOLOv2-03.png)
...
![](https://cdn.jsdelivr.net/gh/makaspacex/PictureZone@main/libs/dlwarm/img/ch8/YOLOv2-04.png)
...
![](https://cdn.jsdelivr.net/gh/makaspacex/PictureZone@main/libs/dlwarm/img/ch8/YOLOv3-01.png)
...
![](https://cdn.jsdelivr.net/gh/makaspacex/PictureZone@main/libs/dlwarm/img/ch8/YOLOv3-02.png)
...
![](https://cdn.jsdelivr.net/gh/makaspacex/PictureZone@main/libs/dlwarm/img/ch8/YOLOv3-03.png)
...
![](https://cdn.jsdelivr.net/gh/makaspacex/PictureZone@main/libs/dlwarm/img/ch8/RetinaNet-01.png)
...
![](https://cdn.jsdelivr.net/gh/makaspacex/PictureZone@main/libs/dlwarm/img/ch8/RetinaNet-02.png)
...
![](https://cdn.jsdelivr.net/gh/makaspacex/PictureZone@main/libs/dlwarm/img/ch8/RetinaNet-03.png)
...
![](https://cdn.jsdelivr.net/gh/makaspacex/PictureZone@main/libs/dlwarm/img/ch8/RetinaNet-04.png)

上式中，y=+1或者y=-1。p∈[0,1]是y=+1的估计概率。作者定义pt为：

![](https://cdn.jsdelivr.net/gh/makaspacex/PictureZone@main/libs/dlwarm/img/ch8/RetinaNet-05.png)

![](https://cdn.jsdelivr.net/gh/makaspacex/PictureZone@main/libs/dlwarm/img/ch8/RetinaNet-06.png)
...
![](https://cdn.jsdelivr.net/gh/makaspacex/PictureZone@main/libs/dlwarm/img/ch8/RetinaNet-07.png)
...
(1-pt)^γ(1−pt)γ
$$
![](https://cdn.jsdelivr.net/gh/makaspacex/PictureZone@main/libs/dlwarm/img/ch8/RetinaNet-08.png)
...
![](https://cdn.jsdelivr.net/gh/makaspacex/PictureZone@main/libs/dlwarm/img/ch8/RetinaNet-09.png)
...
![](https://cdn.jsdelivr.net/gh/makaspacex/PictureZone@main/libs/dlwarm/img/ch8/RetinaNet-10.png)
...
![](https://cdn.jsdelivr.net/gh/makaspacex/PictureZone@main/libs/dlwarm/img/ch8/RetinaNet-11.png)

![](https://cdn.jsdelivr.net/gh/makaspacex/PictureZone@main/libs/dlwarm/img/ch8/RetinaNet-12.png)
...
![](https://cdn.jsdelivr.net/gh/makaspacex/PictureZone@main/libs/dlwarm/img/ch8/RetinaNet-13.png)

![](https://cdn.jsdelivr.net/gh/makaspacex/PictureZone@main/libs/dlwarm/img/ch8/RetinaNet-14.png)
...
![](https://cdn.jsdelivr.net/gh/makaspacex/PictureZone@main/libs/dlwarm/img/ch8/RFBNet-01.png)
...
![](https://cdn.jsdelivr.net/gh/makaspacex/PictureZone@main/libs/dlwarm/img/ch8/RFBNet-02.png)

RFBNet300的整体结构如下图所示，基本上和SSD类似。RFBNet和SSD不同的是：1、主干网上用两个RFB结构替换原来新增的两层。2、conv4_3和conv7_fc在接预测层之前分别接RFB-s和RFB结构。 

![](https://cdn.jsdelivr.net/gh/makaspacex/PictureZone@main/libs/dlwarm/img/ch8/RFBNet-03.png)
...
![](https://cdn.jsdelivr.net/gh/makaspacex/PictureZone@main/libs/dlwarm/img/ch8/M2Det-01.png)
...
![](https://cdn.jsdelivr.net/gh/makaspacex/PictureZone@main/libs/dlwarm/img/ch8/M2Det-02.png)
...
![](https://cdn.jsdelivr.net/gh/makaspacex/PictureZone@main/libs/dlwarm/img/ch8/M2Det-03.png)
...
![](https://cdn.jsdelivr.net/gh/makaspacex/PictureZone@main/libs/dlwarm/img/ch8/8.4.1.png)

图 8.1 图像金字塔           

![](https://cdn.jsdelivr.net/gh/makaspacex/PictureZone@main/libs/dlwarm/img/ch8/8.4.2.png)
...
![](https://cdn.jsdelivr.net/gh/makaspacex/PictureZone@main/libs/dlwarm/img/ch8/8.4.3.png)
...
![](https://cdn.jsdelivr.net/gh/makaspacex/PictureZone@main/libs/dlwarm/img/ch8/8.4.4.png)
...
![](https://cdn.jsdelivr.net/gh/makaspacex/PictureZone@main/libs/dlwarm/img/ch8/8.4.5.png)
...
![](https://cdn.jsdelivr.net/gh/makaspacex/PictureZone@main/libs/dlwarm/img/ch8/8.4.6.png)
...
![](https://cdn.jsdelivr.net/gh/makaspacex/PictureZone@main/libs/dlwarm/img/ch8/8.4.8.png)
...
![](https://cdn.jsdelivr.net/gh/makaspacex/PictureZone@main/libs/dlwarm/img/ch8/8.4.9.png)

![](https://cdn.jsdelivr.net/gh/makaspacex/PictureZone@main/libs/dlwarm/img/ch8/8.4.10.png)

![](https://cdn.jsdelivr.net/gh/makaspacex/PictureZone@main/libs/dlwarm/img/ch8/8.4.11.png)

![](https://cdn.jsdelivr.net/gh/makaspacex/PictureZone@main/libs/dlwarm/img/ch8/8.4.12.png)
...
![](https://cdn.jsdelivr.net/gh/makaspacex/PictureZone@main/libs/dlwarm/img/ch8/8.4.13.png)
...
![](https://cdn.jsdelivr.net/gh/makaspacex/PictureZone@main/libs/dlwarm/img/ch8/8.4.14.png)
...
![](https://cdn.jsdelivr.net/gh/makaspacex/PictureZone@main/libs/dlwarm/img/ch8/8.5.1-1.png)
...
![](https://cdn.jsdelivr.net/gh/makaspacex/PictureZone@main/libs/dlwarm/img/ch8/8.5.1-2.png)
...
* Experiment
![](https://cdn.jsdelivr.net/gh/makaspacex/PictureZone@main/libs/dlwarm/img/ch8/8.5.1-3.png)
...
![](https://cdn.jsdelivr.net/gh/makaspacex/PictureZone@main/libs/dlwarm/img/ch8/8.5.1-4.png)

还有非线性的mixup裁剪如下：  

![](https://cdn.jsdelivr.net/gh/makaspacex/PictureZone@main/libs/dlwarm/img/ch8/8.5.1-5.png)

以及随机裁剪的图像混合：  

![](https://cdn.jsdelivr.net/gh/makaspacex/PictureZone@main/libs/dlwarm/img/ch8/8.5.1-6.png)
...
![](https://cdn.jsdelivr.net/gh/makaspacex/PictureZone@main/libs/dlwarm/img/ch8/8.5.1-7.png)
...
![](https://cdn.jsdelivr.net/gh/makaspacex/PictureZone@main/libs/dlwarm/img/ch8/8.5.1-8.png)
...
![](https://cdn.jsdelivr.net/gh/makaspacex/PictureZone@main/libs/dlwarm/img/ch8/8.5.9-1.png)
...
![](https://cdn.jsdelivr.net/gh/makaspacex/PictureZone@main/libs/dlwarm/img/ch8/8.5.9-2.png)
...
![](https://cdn.jsdelivr.net/gh/makaspacex/PictureZone@main/libs/dlwarm/img/ch8/8.5.9-3.png)
...
![](https://cdn.jsdelivr.net/gh/makaspacex/PictureZone@main/libs/dlwarm/img/ch8/8.5.9-4.png)
...
结构图如下：
![](https://cdn.jsdelivr.net/gh/makaspacex/PictureZone@main/libs/dlwarm/img/ch8/mmdetection.png)
