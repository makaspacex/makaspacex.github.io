...
function ffpp(){
    ...
    newname=${fullname%.*}
    dirr=$(dirname "$1")
    output="$dirr"/2-"$newname".$2
    echo $output
    # ffmpeg  -hwaccel cuvid -i "$1" -c:v copy -c:a copy -movflags +faststart -brand ios6  -y "$output"
    ffmpeg  -hwaccel cuvid -i "$1" -c:v copy -c:a copy -movflags +faststart -brand iosm  -y "$output"
    # ffmpeg -hwaccel cuvid -i "$1" -c:v h264_nvenc -preset fast -crf 18 -b:v 2M -maxrate 10M -bufsize 20M -c:a aac -b:a 256k "$output"
}
